...
set opt(out)        "outputs/out-B[lindex $argv 0]-C[lindex $argv 1]-T[lindex $argv 2]-S[lindex $argv 3]-NodeSpeed.output"     ;# statistics output file
...
set opt(voip-codec)                  [lindex $argv 1];#G.723.1	  ;#.1      ;# VoIP codec
...
set opt(voip-decoder-chain)          [lindex $argv 0] ;# decoders
...
proc finish {} {
	...
	#print the simtime in a text file
        puts $simtimefile "duration:	$simtime s"
	close $simtimefile
	exit 0
}
	...
	set stop [expr $opt(duration)-5]
        #$ns_ duplex-link $n0 $n1 8Mb 10ms DropTail
		  ...
		  for { set i 0 } { $i < 1 } { incr i } {
			  ...
			  set f [create_udp $node_(57) $node_(28) $fid "voip"]
			  if {$opt(voip-bidirectional) != "off"} {
			      ;# create the opposite dir correlated flow
			      create_voip [expr $fid+1] $start $stop
			      create_udp $node_(28) $node_(57) [expr $fid+1] "voip"
			      set f [expr $f+1]
			  }
